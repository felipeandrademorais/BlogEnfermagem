...
function enfermagem_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'enfermagem' ),
 ...
 * @return void
 */
function enfermagem_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'enfermagem' ),
 ...
 * @return bool|string
 */
function enfermagem_the_excerpt( $trim_character_count = 0 ) {
	$post_ID = get_the_ID();
 ...
 */
function enfermagem_excerpt_more( $more = '' ) {

	if ( ! is_single() ) {
		$more = sprintf( '<a class="enfermagem-read-more text-white mt-3 btn btn-info" href="%1$s">%2$s</a>',
			get_permalink( get_the_ID() ),
			__( 'Read more', 'enfermagem' )
		);
	}
...
/**
 * enfermagem Pagination.
 *
 * @return void
 */
function enfermagem_pagination() {
	...
	printf( '<nav class="enfermagem-pagination clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}
 ...
 */
function enfermagem_the_post_pagination( $current_page_no, $posts_per_page, $article_query, $first_page_url, $last_page_url, bool $is_query_param_structure = true ) {
	...
	<div class="mt-0 md:mt-10 mb-10 lg:my-5 flex items-center justify-end posts-navigation">
		...
		if ( 1 !== $current_page_no && ! empty( $first_page_url ) ) {
			printf( '<a class="first-pagination-link btn border border-secondary mr-2" href="%1$s" title="first-pagination-link">%2$s</a>', esc_url( $first_page_url ), __( 'First', 'enfermagem' ) );
		}
		...
		echo paginate_links( [
			...
			'total'     => $total_pages,
			'prev_text' => __( 'Prev', 'enfermagem' ),
			'next_text' => __( 'Next', 'enfermagem' ),
		] );

		// Last Page
		if ( $current_page_no < $total_pages && !empty( $last_page_url ) ) {

			printf( '<a class="last-pagination-link btn border border-secondary ml-2" href="%1$s" title="last-pagination-link">%2$s</a>', esc_url( $last_page_url ), __( 'Last', 'enfermagem' ) );
		}
 ...
 */
function enfermagem_is_uploaded_via_wp_admin( $gravatar_url ) {
 ...
 * @return bool
 */
function enfermagem_has_gravatar( $user_email ) {

	$gravatar_url = get_avatar_url( $user_email );

	if ( enfermagem_is_uploaded_via_wp_admin( $gravatar_url ) ) {
		return true;
	}
